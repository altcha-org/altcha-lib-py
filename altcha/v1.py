...
def create_challenge(
    ...
    *,
    ...
    algorithm = cast(AlgoType, options.algorithm or DEFAULT_ALGORITHM)
...
def verify_solution(
    ...
    # All required fields are present — narrow to a fully-populated view.
    p = cast(dict[str, object], payload_dict)

    if p["algorithm"] not in ["SHA-1", "SHA-256", "SHA-512"]:
...
def extract_params(payload: PayloadType) -> dict[str, list[str]]:
    ...
    """
    split_salt = (payload.get("salt") or "").split("?")
    if len(split_salt) > 1:
...
def verify_server_signature(
    ...
    p = cast(dict[str, object], payload_dict)
    algorithm = cast(AlgoType, p["algorithm"])
    verification_data = cast(str, p["verificationData"])
    signature = cast(str, p["signature"])
    verified = cast(bool, p["verified"])
